    ...
    <div className="fixed inset-0 z-50 bg-midnight flex flex-col items-center justify-center p-8 text-center overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          ...
          >
            ...
            <h2 className="text-2xl text-white font-bold mb-6">Avant de commencer</h2>
            ...
            <div className="bg-rose-900/10 border border-rose-900/30 rounded-xl p-6 mb-8 text-left">
              <p className="text-rose-200/90 text-sm leading-relaxed mb-4">
                <strong className="text-rose-400 block mb-2 text-base">LUCID n'est pas un médecin.</strong>
            ...
            <button
              onClick={nextStep}
              className="w-full py-4 rounded-2xl bg-surface border border-slate-700 text-white font-bold text-lg hover:bg-slate-800 transition-all"
            >
              J'ai compris
            </button>
          </motion.div>
      ...
      <div className="absolute bottom-10 flex gap-3">
        {[0, 1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`h-2 rounded-full transition-all duration-300 ${i === step ? 'w-8 bg-accent' : 'w-2 bg-slate-800'
              }`}
          />
        ))}
      </div>
